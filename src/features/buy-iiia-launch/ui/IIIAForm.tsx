...
import { IIIAFormLaunchStore } from "../model";
...
export const IIIAFormLaunch: FC = observer(() => {
  const { t } = useTranslation();
    const rootStore = useRootStore()
  const dcon = useAccount();
  const { refCode } = useRootStore();
  const [store] = useState(
    () => new IIIAFormLaunchStore(rootStore, refCode, dcon.address)
  );
  ...
  return (
    <>
      <BaseTokensForm
        title={t("common.purchaseToken", { symbol: TOKEN_SYMBOLS.IIIA })}
        onSubmit={onSubmit}
